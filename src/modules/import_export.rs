...
pub fn export(world_objects: &Vec<WorldObject>) {
  ...
  file.write_record(&["id", "model", "location", "x", "y", "z", "rot_x", "rot_y", "rot_z", "size_x", "size_y", "size_z"]).unwrap();
...
pub fn import() -> (Vec<(String, String)>, Vec<WorldObject>) {
  ...
  for whole_object in reader.records() {
    ...
    let location: String = object[2].parse().unwrap();
    let x: f32 = object[3].parse().unwrap();
    let y: f32 = object[4].parse().unwrap();
    let z: f32 = object[5].parse().unwrap();
    let rot_x: f32 = object[6].parse().unwrap();
    let rot_y: f32 = object[7].parse().unwrap();
    let rot_z: f32 = object[8].parse().unwrap();
    let size_x: f32 = object[9].parse().unwrap();
    let size_y: f32 = object[10].parse().unwrap();
    let size_z: f32 = object[11].parse().unwrap();
    ...
    world_objects.push(WorldObject::new(id, model, location, 
                                        Vector3::new(x, y, z),
